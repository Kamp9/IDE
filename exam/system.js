...
// Create Slider
    ...
    slider.append("line")
        ...
        .call(d3.drag()
            ...
            .on("start drag", function () {
                select_planets(x.invert(d3.event.x));
            }));
    ...
    function select_planets(h) {
        ...
        switch (slider_mode) {
            case 0:
                container.selectAll("g.planet").filter(function (d, i) {
                    return i < num_planets
                }).attr("opacity", 1)
                container.selectAll("g.planet").filter(function (d, i) {
                    return i >= num_planets
                }).attr("opacity", 0)
            ...
            case 1:
                container.selectAll("g.planet").filter(function (d, i) {
                    return d['Norm Distance'] < num_planets / 100
                }).attr("opacity", 1)
                container.selectAll("g.planet").filter(function (d, i) {
                    return d['Norm Distance'] > num_planets / 100
                }).attr("opacity", 0)
            ...
            case 2:
                ...
                container.selectAll("g.planet").filter(function (d, i) {
                    return (d['P. Ts Mean (K)'] - t_min) / (t_max - t_min) < num_planets / 100
                }).attr("opacity", 1)
                container.selectAll("g.planet").filter(function (d, i) {
                    return (d['P. Ts Mean (K)'] - t_min) / (t_max - t_min) > num_planets / 100
                }).attr("opacity", 0)
            ...
            case 3:
                ...
                container.selectAll("g.planet").filter(function (d, i) {
                    return (d['P. Name'] == "KOI-571.05")
                }).attr("opacity", 1);
                break;
            case 4:
                container.selectAll("g.planet").attr("opacity", 0)
                container.selectAll("g.planet").filter(function (d, i) {
                    return (d['P. Name'] == "KOI-87.01")
                }).attr("opacity", 1);
                break;
            case 5:
                container.selectAll("g.planet").attr("opacity", 0)
                container.selectAll("g.planet").filter(function (d, i) {
                    return (d['P. Name'] == "KOI-701.03")
                }).attr("opacity", 1);
                // container.selectAll("g.planet").filter(function(d, i) { return (d['P. Name']=="KOI-701.03")}).select("circle.orbit").attr("opacity",1);
                break;
            default:
        }
...
// console.log(left1);
    ...
    function change_dataset(use_dataset) {
        ...
        container.selectAll("g.planet").data(use_data).enter().append("g")
            ...
            // d3.select(this).append("circle").attr("class","orbit").attr("r",(0.05+d['Norm Distance'])*h/2).attr("fill","none").attr("stroke","#ffffff").attr("opacity",0);
        });
